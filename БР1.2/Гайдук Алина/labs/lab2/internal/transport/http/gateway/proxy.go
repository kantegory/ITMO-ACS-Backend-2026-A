...
func newReverseProxy(name, rawURL string) http.Handler {
	...
	if err != nil || target.Scheme == "" || target.Host == "" {
		return http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			response.RespondError(w, http.StatusBadGateway, "bad_gateway", "upstream is not configured")
		})
	}
	...
	proxy.ErrorHandler = func(w http.ResponseWriter, r *http.Request, err error) {
		...
		response.RespondError(w, http.StatusBadGateway, "bad_gateway", "upstream is unavailable")
	}
