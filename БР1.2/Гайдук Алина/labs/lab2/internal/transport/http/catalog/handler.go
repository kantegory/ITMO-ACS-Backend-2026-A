...
func (h *Handler) ListDishTypes(w http.ResponseWriter, r *http.Request) {
	...
	response.RespondJSON(w, http.StatusOK, map[string]any{"data": out})
}
...
func (h *Handler) CreateDishType(w http.ResponseWriter, r *http.Request) {
	...
	response.RespondJSON(w, http.StatusCreated, map[string]any{"data": toDishTypeResponse(row)})
}
...
func (h *Handler) CreateTag(w http.ResponseWriter, r *http.Request) {
	...
	response.RespondJSON(w, http.StatusCreated, map[string]any{"data": toTagResponse(row)})
}
...
func (h *Handler) CreateIngredient(w http.ResponseWriter, r *http.Request) {
	...
	response.RespondJSON(w, http.StatusCreated, map[string]any{"data": toIngredientResponse(row)})
}
...
func (h *Handler) ValidateIDs(w http.ResponseWriter, r *http.Request) {
	...
	response.RespondJSON(w, http.StatusOK, toValidateIDsResponse(result))
}
...
func respondCreateError(w http.ResponseWriter, err error, conflictMessage string) bool {
	...
	if errors.Is(err, catalogusecase.ErrInvalidName) {
		response.RespondError(w, http.StatusUnprocessableEntity, "VALIDATION_ERROR", "name must be non-empty and within length limit")
		return true
	}
	if errors.Is(err, catalogusecase.ErrAlreadyExists) {
		response.RespondError(w, http.StatusConflict, "CONFLICT", conflictMessage)
		return true
	}
