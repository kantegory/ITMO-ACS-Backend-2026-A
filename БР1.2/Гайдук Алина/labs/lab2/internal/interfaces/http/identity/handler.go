...
func (h *Handler) Register(w http.ResponseWriter, r *http.Request) {
	...
	api.RespondJSON(w, http.StatusCreated, map[string]any{"data": toTokenPairResponse(pair)})
}
...
func (h *Handler) Login(w http.ResponseWriter, r *http.Request) {
	...
	api.RespondJSON(w, http.StatusOK, map[string]any{"data": toTokenPairResponse(pair)})
}
...
func (h *Handler) Follow(w http.ResponseWriter, r *http.Request) {
	...
	api.RespondJSON(w, http.StatusCreated, map[string]any{"data": followingResponse{Following: true}})
}
...
func (h *Handler) Unfollow(w http.ResponseWriter, r *http.Request) {
	...
	api.RespondJSON(w, http.StatusOK, map[string]any{"data": followingResponse{Following: false}})
}
...
func (h *Handler) UserShort(w http.ResponseWriter, r *http.Request) {
	...
	api.RespondJSON(w, http.StatusOK, toUserShortResponse(user))
}
...
func (h *Handler) UsersBatch(w http.ResponseWriter, r *http.Request) {
	...
	api.RespondJSON(w, http.StatusOK, batchUsersResponse{Users: out})
}
...
func respondAuthMutationError(w http.ResponseWriter, err error) bool {
	...
	if errors.Is(err, identityusecase.ErrEmailExists) {
		api.RespondError(w, http.StatusConflict, "CONFLICT", "user with this email already exists")
		return true
	}
	if errors.Is(err, identityusecase.ErrUsernameExists) {
		api.RespondError(w, http.StatusConflict, "CONFLICT", "user with this username already exists")
		return true
	}
	if errors.Is(err, identityusecase.ErrInvalidCredentials) || errors.Is(err, identityusecase.ErrInvalidRefresh) {
		api.RespondError(w, http.StatusUnauthorized, "UNAUTHORIZED", "invalid credentials")
		return true
	}
...
func respondFollowError(w http.ResponseWriter, err error) bool {
	...
	if errors.Is(err, identityusecase.ErrSelfFollow) {
		api.RespondError(w, http.StatusBadRequest, "BAD_REQUEST", "cannot follow yourself")
		return true
	}
	if errors.Is(err, identityusecase.ErrAlreadyFollowing) {
		api.RespondError(w, http.StatusConflict, "CONFLICT", "already following this user")
		return true
	}
