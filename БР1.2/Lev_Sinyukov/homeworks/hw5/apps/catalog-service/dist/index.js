...
async function bootstrap() {
    ...
    await (0, shared_1.registerRpcHandler)(channel, "catalog.restaurants.search.queue", "catalog.restaurants.search.request", async ({ name, cuisine, city, min_price_range, max_price_range }) => {
        const filtered = restaurants.filter((restaurant) => {
            if (name && !restaurant.name.toLowerCase().includes(name.trim().toLowerCase())) {
                return false;
            }
            if (cuisine && !restaurant.cuisines.some((item) => item.toLowerCase() === cuisine.trim().toLowerCase())) {
                return false;
            }
    ...
    await (0, shared_1.registerRpcHandler)(channel, "catalog.table.validate.queue", "catalog.table.validate.request", async ({ restaurant_id, table_id }) => {
        const restaurant = restaurants.find((item) => item.id === Number(restaurant_id));
        const table = restaurant?.tables.find((item) => item.id === Number(table_id));
        return {
            ok: true,
