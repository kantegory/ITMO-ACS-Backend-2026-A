...
export async function refresh(req: Request, res: Response) {
  ...
  const user = row.user;
  res.json(authResponse(user, refresh_token));
}
...
export async function resetPassword(req: Request, res: Response) {
  ...
  await userRepo.save(row.user);
  await repo.delete({ id: row.id });
  await AppDataSource.getRepository(RefreshToken).delete({ userId: row.user.id });
  res.json({ ok: true });
}
