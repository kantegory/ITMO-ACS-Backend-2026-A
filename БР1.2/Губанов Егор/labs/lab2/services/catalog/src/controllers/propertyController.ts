...
import { getUserBrief } from "../../../../packages/shared/src/clients";
import {
  propertyShort,
  propertyDetail,
  photoOut,
...
function canManage(user: AuthUser, property: Property) {
  return user.role === "ADMIN" || property.ownerId === user.id;
}

function isLandlord(user: AuthUser) {
...
async function loadPropertyFull(id: string) {
  return AppDataSource.getRepository(Property).findOne({
    where: { id },
    relations: ["photos", "conditions", "type"],
  });
}
...
export async function listPublic(req: Request, res: Response) {
  ...
  const [items, total] = await qb
    ...
    .take(pageSize)
    .getManyAndCount();
  res.json({ items: items.map(propertyShort), total });
}
...
export async function getPublic(req: Request, res: Response) {
  ...
  const owner = await getUserBrief(p.ownerId);
  if (!owner) throw E.notFound();
  res.json(propertyDetail(p, owner));
}
...
export async function create(req: Request, res: Response) {
  ...
  const owner = await getUserBrief(user.id);
  if (!owner) throw E.unauthorized();
  res.json(propertyDetail(full!, owner));
}
...
export async function update(req: Request, res: Response) {
  ...
  const full = await loadPropertyFull(p.id);
  const owner = await getUserBrief(full!.ownerId);
  if (!owner) throw E.notFound();
  res.json(propertyDetail(full!, owner));
}
...
export async function addPhoto(req: Request, res: Response) {
  ...
  if (is_main) await repo.update({ propertyId: p.id }, { isMain: false });
...
export async function addCondition(req: Request, res: Response) {
  ...
  let sort_order =
    b.sort_order != null ? parseInt(String(b.sort_order), 10) : NaN;
  ...
  const c = repo.create({ propertyId: p.id, text, sortOrder: sort_order });
  await repo.save(c);
