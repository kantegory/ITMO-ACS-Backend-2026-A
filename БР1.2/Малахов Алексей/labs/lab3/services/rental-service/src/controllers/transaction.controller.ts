import { Get, Post, Body, QueryParam, Req, Res, UseBefore, HttpCode } from 'routing-controllers';
import { OpenAPI } from 'routing-controllers-openapi';
import { IsInt, IsNumber, IsOptional, IsEnum, IsDateString } from 'class-validator';
...
import { TransactionType, TransactionStatus, PaymentMethod, CurrencyType } from '../models/enums';
...
class CreateTransactionDto {
    ...
    @IsDateString() @IsOptional() period_start?: string;
    @IsDateString() @IsOptional() period_end?: string;
}
...
class TransactionController {
    ...
    @OpenAPI({ summary: 'История транзакций пользователя', security: [{ bearerAuth: [] }] })
    async list(
    ...
    async create(@Req() req: RequestWithUser, @Body({ type: CreateTransactionDto }) dto: CreateTransactionDto, @Res() res: Response) {
        const rental = await dataSource.getRepository(Rental).findOneBy({ id: dto.rental_id });
        if (!rental) return res.status(404).json({ code: 'NOT_FOUND', message: 'Аренда не найдена' });
        if (rental.renterId !== req.user.id) return res.status(403).json({ code: 'FORBIDDEN', message: 'Нет доступа' });
        ...
        const transaction = repo.create({
            ...
            periodStart: dto.period_start ?? null, periodEnd: dto.period_end ?? null,
