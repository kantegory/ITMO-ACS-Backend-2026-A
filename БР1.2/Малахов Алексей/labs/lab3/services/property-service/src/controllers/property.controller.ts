import {
    Get, Post, Patch, Delete, Body, Param, QueryParam, Req, Res, UseBefore, HttpCode,
...
import { PropertyType, PropertyStatus, CurrencyType } from '../models/enums';
...
class CreatePropertyDto {
    ...
    @IsString() @IsOptional() postal_code?: string;
    @IsString() city: string;
    @IsString() country: string;
    @IsNumber() @IsOptional() @Type(() => Number) latitude?: number;
    @IsNumber() @IsOptional() @Type(() => Number) longitude?: number;
    @IsInt() @IsOptional() @Type(() => Number) rooms?: number;
    @IsNumber() @IsOptional() @Type(() => Number) area_sqm?: number;
    @IsInt() @IsOptional() @Type(() => Number) floor?: number;
    @IsNumber() @Type(() => Number) price_per_month: number;
    @IsEnum(CurrencyType) @IsOptional() currency?: CurrencyType;
    @IsString() @IsOptional() rental_conditions?: string;
}

class UpdatePropertyDto {
    @IsString() @IsOptional() title?: string;
    @IsString() @IsOptional() description?: string;
    @IsNumber() @IsOptional() @Type(() => Number) price_per_month?: number;
    @IsString() @IsOptional() rental_conditions?: string;
...
class AddPhotoDto {
    @IsString() url: string;
    @IsBoolean() @IsOptional() @Type(() => Boolean) is_main?: boolean;
    @IsInt() @IsOptional() @Type(() => Number) sort_order?: number;
}
...
async function fetchOwnerInfo(ownerId: number) {
    try {
        const res = await fetch(`${SETTINGS.USER_SERVICE_URL}/internal/users/${ownerId}`, {
...
async function fetchLandlordRating(landlordId: number) {
    try {
        const res = await fetch(`${SETTINGS.REVIEW_SERVICE_URL}/internal/reviews/landlord-rating/${landlordId}`, {
            headers: { 'X-Service-Token': SETTINGS.SERVICE_TOKEN },
        });
        if (!res.ok) return { average_rating: null, reviews_count: 0 };
        return await res.json() as any;
    } catch { return { average_rating: null, reviews_count: 0 }; }
}
...
class PropertyController {
    ...
    async create(@Req() req: RequestWithUser, @Body({ type: CreatePropertyDto }) dto: CreatePropertyDto, @Res() res: Response) {
        ...
        const property = repo.create({
            type: dto.type, title: dto.title, description: dto.description ?? null,
            street: dto.street, building: dto.building, apartment: dto.apartment ?? null,
            postalCode: dto.postal_code ?? null, city: dto.city, country: dto.country,
            latitude: dto.latitude ?? null, longitude: dto.longitude ?? null,
            rooms: dto.rooms ?? null, areaSqm: dto.area_sqm ?? null, floor: dto.floor ?? null,
            pricePerMonth: dto.price_per_month, currency: dto.currency ?? CurrencyType.RUB,
            rentalConditions: dto.rental_conditions ?? null,
            ownerId: req.user.id, status: PropertyStatus.ACTIVE,
        });
        await repo.save(property);

        const histRepo = dataSource.getRepository(PropertyPriceHistory);
        await histRepo.save(histRepo.create({ propertyId: property.id, pricePerMonth: property.pricePerMonth, currency: property.currency }));

        return res.status(201).json(await this._buildDetail(property.id, req.user.id));
    }

    @Get('/:id')
    @OpenAPI({ summary: 'Получить детали объекта' })
    async getById(@Param('id') id: number, @Req() req: any, @Res() res: Response) {
        const userId = tryGetUserIdFromAuth(req.headers?.authorization);
        const detail = await this._buildDetail(id, userId ?? undefined);
        if (!detail) return res.status(404).json({ code: 'NOT_FOUND', message: 'Объект не найден' });
    ...
    async update(@Param('id') id: number, @Req() req: RequestWithUser, @Body({ type: UpdatePropertyDto }) dto: UpdatePropertyDto, @Res() res: Response) {
        const repo = dataSource.getRepository(Property);
        const property = await repo.findOneBy({ id });
        if (!property) return res.status(404).json({ code: 'NOT_FOUND', message: 'Объект не найден' });
        if (property.ownerId !== req.user.id) return res.status(403).json({ code: 'FORBIDDEN', message: 'Нет доступа' });
        ...
        if (dto.price_per_month !== undefined) property.pricePerMonth = dto.price_per_month;
        if (dto.rental_conditions !== undefined) property.rentalConditions = dto.rental_conditions;
    ...
    @Delete('/:id')
    @UseBefore(authMiddleware)
    @OpenAPI({ summary: 'Удалить объект (soft delete)', security: [{ bearerAuth: [] }] })
    async remove(@Param('id') id: number, @Req() req: RequestWithUser, @Res() res: Response) {
        const repo = dataSource.getRepository(Property);
        const property = await repo.findOneBy({ id });
        if (!property) return res.status(404).json({ code: 'NOT_FOUND', message: 'Объект не найден' });
        if (property.ownerId !== req.user.id) return res.status(403).json({ code: 'FORBIDDEN', message: 'Нет доступа' });
    ...
    async addPhoto(@Param('id') id: number, @Req() req: RequestWithUser, @Body({ type: AddPhotoDto }) dto: AddPhotoDto, @Res() res: Response) {
        ...
        if (count >= 20) return res.status(400).json({ code: 'PHOTO_LIMIT_EXCEEDED', message: 'Превышен лимит фото (20)' });

        const photo = photoRepo.create({ propertyId: id, url: dto.url, isMain: dto.is_main ?? false, sortOrder: dto.sort_order ?? count + 1 });
    ...
    async deletePhoto(@Param('propertyId') propertyId: number, @Param('photoId') photoId: number, @Req() req: RequestWithUser, @Res() res: Response) {
        ...
        if (!photo) return res.status(404).json({ code: 'NOT_FOUND', message: 'Фото не найдено' });
        await photoRepo.delete(photoId);
        return res.status(204).send();
    }

    private async _buildDetail(propertyId: number, currentUserId?: number) {
        const property = await dataSource.getRepository(Property).findOne({
            where: { id: propertyId },
        ...
        if (currentUserId) {
            const fav = await dataSource.getRepository(Favorite).findOneBy({ userId: currentUserId, propertyId });
            isFavorited = !!fav;
        }

        const [owner, ratingData] = await Promise.all([
        ...
        return {
            ...
            address: {
                ...
                apartment: property.apartment ?? null, postal_code: property.postalCode ?? null,
