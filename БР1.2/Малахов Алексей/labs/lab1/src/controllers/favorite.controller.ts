...
@EntityController({ baseRoute: "/favorites", entity: Favorite })
class FavoriteController extends BaseController {
  @Get("")
  @UseBefore(authMiddleware)
  @OpenAPI({
    summary: "Список избранных объектов",
    security: [{ bearerAuth: [] }],
  })
  async list(
    ...
    return res.json({
      items: items.map((f) => ({
        ...
        price_per_month: f.property.pricePerMonth,
        currency: f.property.currency,
        rooms: f.property.rooms ?? null,
  ...
  async add(
    ...
    if (existing) {
      return res
        .status(409)
        .json({ code: "ALREADY_FAVORITED", message: "Уже в избранном" });
    }

    const fav = repo.create({ userId: req.user.id, propertyId });
    await repo.save(fav);
  ...
  async remove(
    ...
    if (!fav)
      return res
        .status(404)
        .json({ code: "NOT_FOUND", message: "Не в избранном" });
