...
export class PropertyPriceHistory extends BaseEntity {
    ...
    @Column({ type: 'decimal', precision: 12, scale: 2, nullable: false, transformer: decimalTransformer }) pricePerMonth: number;
    @Column({ type: 'enum', enum: CurrencyType, nullable: false }) currency: CurrencyType;
    @CreateDateColumn() changedAt: Date;
    @ManyToOne(() => Property, (p) => p.priceHistory, { onDelete: 'CASCADE' })
