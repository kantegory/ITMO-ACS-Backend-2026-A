...
class Settings {
    ...
    APP_PORT: number = parseInt(env.APP_PORT) || 3006;
    ...
    DB_PORT = parseInt(env.DB_PORT || env.POSTGRES_PORT) || 15437;
    DB_NAME = env.DB_NAME || env.POSTGRES_DB || 'applications_db';
    ...
    // jwt settings
    JWT_SECRET_KEY = env.JWT_SECRET_KEY || env.JWT_SECRET || 'secret';
    JWT_TOKEN_TYPE = env.JWT_TOKEN_TYPE || 'Bearer';
    JWT_ACCESS_TOKEN_LIFETIME: number =
        parseInt(env.JWT_ACCESS_TOKEN_LIFETIME) || 60 * 5;
    SERVICE_TOKEN = env.SERVICE_TOKEN || 'dev-service-token';
    VACANCY_SERVICE_URL =
        env.VACANCY_SERVICE_URL || 'http://localhost:3004/api/v1';
    RESUME_SERVICE_URL =
        env.RESUME_SERVICE_URL || 'http://localhost:3005/api/v1';
    COMPANY_SERVICE_URL =
        env.COMPANY_SERVICE_URL || 'http://localhost:3002/api/v1';
    RABBITMQ_URL = env.RABBITMQ_URL || 'amqp://guest:guest@localhost:5672';
    RABBITMQ_EXCHANGE = env.RABBITMQ_EXCHANGE || 'job-platform.events';
}
