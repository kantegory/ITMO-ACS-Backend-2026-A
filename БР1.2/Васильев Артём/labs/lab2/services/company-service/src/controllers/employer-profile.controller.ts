...
class EmployerProfileController extends BaseController {
    ...
    async create(
        ...
        const users = await serviceBatchGet<{ id: string }>(
            SETTINGS.AUTH_USER_SERVICE_URL,
            '/internal/v1/users/batch',
            [request.user.id],
        );
        ensureFound(
            users.items.find((user) => user.id === request.user.id),
            'User not found',
        );
    ...
    async list(
        ...
        if (query.search) {
            qb.andWhere(
                '(profile.position ILIKE :search OR company.title ILIKE :search)',
                { search: `%${query.search}%` },
            );
        }
