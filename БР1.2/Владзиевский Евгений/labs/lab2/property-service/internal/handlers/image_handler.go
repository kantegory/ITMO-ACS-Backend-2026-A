...
func NewImageHandler(cfg *config.Config, authClient *client.AuthClient) *ImageHandler {
	imageRepo := repositories.NewPropertyImageRepository()
	propertyRepo := repositories.NewPropertyRepository()
	imageService := services.NewPropertyImageService(imageRepo, propertyRepo, authClient, cfg)
	return &ImageHandler{
...
func (h *ImageHandler) Upload(c *gin.Context) {
	...
	if err := c.Request.ParseMultipartForm(h.cfg.MaxUploadSize); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": gin.H{"code": "VALIDATION_ERROR", "message": "Failed to parse form"},
		})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": gin.H{"code": "VALIDATION_ERROR", "message": "Missing image file"},
		})
		return
	}
	defer file.Close()
...
func (h *ImageHandler) List(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": gin.H{"code": "INTERNAL_ERROR", "message": "Failed to retrieve images"},
		})
		return
	}
...
func (h *ImageHandler) Delete(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": gin.H{"code": "VALIDATION_ERROR", "message": "Invalid image ID"},
		})
		return
	}
...
func (h *ImageHandler) SetMain(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(status, gin.H{
			"error": gin.H{"code": "SET_MAIN_ERROR", "message": err.Error()},
		})
		return
	}
