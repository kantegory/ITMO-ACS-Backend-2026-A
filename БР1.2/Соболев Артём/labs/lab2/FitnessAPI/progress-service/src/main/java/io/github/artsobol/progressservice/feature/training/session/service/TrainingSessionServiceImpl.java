...
import io.github.artsobol.progressservice.feature.training.session.mapper.TrainingSessionMapper;
...
public class TrainingSessionServiceImpl implements TrainingSessionService, TrainingSessionFinder {
    ...
    public TrainingSessionResponse create(Long trainingId, Long userId) {
        ...
        RemoteTrainingResponse training = trainingServiceClient.getTrainingById(trainingId);

        TrainingSession entity = TrainingSession.create(userId, trainingId);
        for (RemoteTrainingExerciseResponse exercise : training.exercises()) {
            entity.addExercise(exercise.id());
        }
