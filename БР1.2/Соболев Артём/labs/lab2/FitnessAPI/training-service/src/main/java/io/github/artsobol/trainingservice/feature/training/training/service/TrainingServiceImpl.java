...
import io.github.artsobol.trainingservice.feature.training.exercise.service.TrainingExerciseFinder;
import io.github.artsobol.trainingservice.feature.training.tag.entity.Tag;
import io.github.artsobol.trainingservice.feature.training.tag.service.TagFinder;
import io.github.artsobol.trainingservice.feature.training.training.dto.request.CreateTrainingRequest;
import io.github.artsobol.trainingservice.feature.training.training.dto.request.UpdateTrainingRequest;
...
import io.github.artsobol.trainingservice.feature.training.training.mapper.TrainingMapper;
import io.github.artsobol.trainingservice.feature.training.training.repository.TrainingRepository;
import io.github.artsobol.trainingservice.feature.training.type.entity.Type;
import io.github.artsobol.trainingservice.feature.training.type.service.TypeFinder;
import io.github.artsobol.trainingservice.integration.training.rabbit.TrainingCatalogEventPublisher;
...
public class TrainingServiceImpl implements TrainingService, TrainingFinder {
    ...
    private final TrainingCatalogEventPublisher trainingCatalogEventPublisher;
    ...
    @Override
    @Transactional(readOnly = true)
    public Slice<TrainingResponse> getAll(Pageable pageable, String name) {
        log.debug(
                "Fetching trainings page={} size={} sort={} name={}",
        ...
        if (hasSearchName(name)) {
            return trainingRepository.findByIsActiveTrueAndTitleContainingIgnoreCase(name.strip(), pageable)
    ...
    public TrainingResponse create(CreateTrainingRequest request, Long authorId) {
        ...
        Training entity = Training.create(authorId, request.title(), request.description(), request.trainingLevel());
        trainingRepository.save(entity);
        trainingRepository.flush();
        trainingCatalogEventPublisher.publishChanged(entity);

        log.info("Training created trainingId={} authorId={}", entity.getId(), entity.getAuthorId());
        return trainingMapper.toResponse(entity);
    }
    ...
    public TrainingResponse addExercise(Long trainingId, Long exerciseId) {
        ...
        Training entity = findByIdOrThrow(trainingId);
        entity.addExercise(exerciseId);
        trainingRepository.flush();
        trainingCatalogEventPublisher.publishChanged(entity);

        log.info("Exercise added trainingId={} exerciseId={}", entity.getId(), exerciseId);
        return trainingMapper.toResponse(entity);
    }
