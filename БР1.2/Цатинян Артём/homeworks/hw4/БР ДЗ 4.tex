...
\begin{tikzpicture}[
    ...
    db/.style={cylinder, draw, shape border rotate=90, aspect=0.25, align=center, minimum width=2.4cm, minimum height=1cm},
    arrow/.style={-{Latex}, thick}
]
    \node[service] (client) {Client\\Postman / Web};
    \node[service, right=1.8cm of client] (gateway) {API Gateway};
    \node[service, above right=1.1cm and 1.8cm of gateway] (identity) {Identity\\Service};
    \node[service, right=1.8cm of gateway] (catalog) {Restaurant Catalog\\Service};
    \node[service, below right=1.1cm and 1.8cm of gateway] (booking) {Booking\\Service};
    \node[service, below=1.4cm of catalog] (review) {Review\\Service};

    \node[db, right=1.2cm of identity] (identitydb) {\code{identity\_db}};
    \node[db, right=1.2cm of catalog] (catalogdb) {\code{catalog\_db}};
    \node[db, right=1.2cm of booking] (bookingdb) {\code{booking\_db}};
    \node[db, right=1.2cm of review] (reviewdb) {\code{review\_db}};

    \draw[arrow] (client) -- (gateway);
    \draw[arrow] (gateway) -- (identity);
    \draw[arrow] (gateway) -- (catalog);
    \draw[arrow] (gateway) -- (booking);
    \draw[arrow] (gateway) -- (review);
    ...
    \draw[arrow, dashed] (booking) -- node[right, font=\small]{internal REST} (catalog);
    \draw[arrow, dashed] (review) -- node[left, font=\small]{review context} (booking);
    \draw[arrow, dashed] (review) -- node[above, font=\small]{user summary} (identity);
    \draw[arrow, dashed] (review) -- node[right, font=\small]{ReviewCreated event} (catalog);
\end{tikzpicture}
\end{center}

Основной тип синхронного взаимодействия между сервисами --- REST по внутренним эндпоинтам. Для обновления агрегированного рейтинга ресторана используется асинхронное событие \code{ReviewCreated}, потому что рейтинг не должен блокировать создание отзыва распределенной транзакцией.
...
\hline
\code{POST /api/v1/auth/register} & Identity Service \\
\hline
\code{POST /api/v1/auth/login} & Identity Service \\
\hline
\code{GET /api/v1/users/me}, \code{PATCH /api/v1/users/me} & Identity Service \\
\hline
\code{GET /api/v1/cuisines} & Restaurant Catalog Service \\
\hline
\code{GET /api/v1/restaurants} & Restaurant Catalog Service \\
\hline
\code{GET /api/v1/restaurants/\{restaurantId\}} & Restaurant Catalog Service \\
\hline
\code{GET /api/v1/restaurants/\{restaurantId\}/menu} & Restaurant Catalog Service \\
\hline
\code{GET /api/v1/restaurants/\{restaurantId\}/availability} & Booking Service \\
\hline
\code{POST /api/v1/bookings} & Booking Service \\
\hline
\code{GET /api/v1/bookings/me} & Booking Service \\
\hline
\code{GET /api/v1/bookings/\{bookingId\}} & Booking Service \\
\hline
\code{PATCH /api/v1/bookings/\{bookingId\}/cancel} & Booking Service \\
\hline
\code{GET /api/v1/restaurants/\{restaurantId\}/reviews} & Review Service \\
\hline
\code{POST /api/v1/restaurants/\{restaurantId\}/reviews} & Review Service \\
\hline
\end{longtable}
...
\begin{longtable}{|p{0.44\textwidth}|p{0.22\textwidth}|p{0.24\textwidth}|}
\hline
\textbf{Endpoint} & \textbf{Кто вызывает} & \textbf{Назначение} \\
\hline
\code{GET /identity/internal/v1/auth/jwks} & API Gateway и сервисы & Получение публичных ключей для проверки JWT \\
\hline
\code{GET /identity/internal/v1/users/\{userId\}/summary} & Review Service, Booking Service & Получение краткого профиля пользователя \\
\hline
\code{GET /catalog/internal/v1/restaurants/\{restaurantId\}/summary} & Booking Service, Review Service & Получение краткой информации о ресторане \\
\hline
\code{GET /catalog/internal/v1/restaurants/\{restaurantId\}/booking-context} & Booking Service & Проверка ресторана, столика, вместимости и рабочих часов перед созданием бронирования \\
\hline
\code{GET /catalog/internal/v1/restaurants/\{restaurantId\}/availability-context} & Booking Service & Получение активных столиков и рабочих часов для расчета доступности \\
\hline
\code{POST /booking/internal/v1/bookings/availability/check} & Restaurant Catalog Service или API Gateway & Проверка занятости набора столиков на интервал времени \\
\hline
\code{GET /booking/internal/v1/bookings/\{bookingId\}/review-context} & Review Service & Проверка права пользователя оставить отзыв \\
\hline
\code{GET /reviews/internal/v1/restaurants/\{restaurantId\}/rating-stats} & Restaurant Catalog Service & Синхронизация рейтингов при пересчете или восстановлении read-model \\
\hline
\end{longtable}
...
\begin{enumerate}[leftmargin=1.7cm]
    \item Клиент отправляет \code{POST /api/v1/bookings} в API Gateway.
    \item Gateway проверяет JWT и передает запрос в Booking Service вместе с \code{userId}.
    \item Booking Service вызывает \code{GET /catalog/internal/v1/restaurants/\{restaurantId\}/booking-context}.
...
\begin{enumerate}[leftmargin=1.7cm]
    \item Клиент отправляет \code{GET /api/v1/restaurants/\{restaurantId\}/availability}.
...
\begin{enumerate}[leftmargin=1.7cm]
    \item Клиент отправляет \code{POST /api/v1/restaurants/\{restaurantId\}/reviews}.
    \item Gateway проверяет JWT и передает запрос в Review Service.
    \item Review Service вызывает \code{GET /booking/internal/v1/bookings/\{bookingId\}/review-context}.
...
\begin{enumerate}[leftmargin=1.7cm]
    \item Клиент отправляет \code{GET /api/v1/restaurants} с фильтрами, сортировкой и пагинацией.
    ...
    \item Если read-model нужно восстановить, Catalog Service может запросить \code{GET /reviews/internal/v1/restaurants/\{restaurantId\}/rating-stats}.
\end{enumerate}
