...
app.patch('/internal/restaurants/:restaurantId/rating-summary', asyncHandler(async (request, response) => {
    const restaurant = await getRestaurantOrFail(getParam(request.params.restaurantId, 'restaurantId'), false);
    const avgRating = Number(request.body?.avgRating || 0);
    const reviewsCount = Number(request.body?.reviewsCount || 0);
    ...
    await restaurants().save(restaurant);
    response.send({ data: { restaurantId: restaurant.id, avgRating, reviewsCount } });
}));
