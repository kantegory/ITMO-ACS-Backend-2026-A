const baseUrl = process.env.GATEWAY_URL || 'http://127.0.0.1:8200/api/v1';
...
await step('create review and update reservation status', async () => {
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    const restaurant = await request('GET', `/restaurants/${state.restaurantId}`);
    if (restaurant.data.avgRating === 5 && restaurant.data.reviewsCount === 1) {
      ratingUpdated = true;
      break;
    }
    await sleep(500);
  ...
  if (!ratingUpdated) {
    throw new Error('Restaurant rating was not updated through RabbitMQ event');
  }
