...
class ReservationController extends BaseController {
    ...
    async create(
        ...
        @Body({ type: CreateReservationDto }) body: CreateReservationDto,
    ) {
        const { user } = request;
        ...
        if (conflicting) {
            return { message: 'Table is already booked for this time' };
        }
    ...
    async update(
        @Param('id') id: number,
        @Req() request: RequestWithUser,
        @Body({ type: UpdateReservationDto }) body: UpdateReservationDto,
    ) {
        const { user } = request;
        ...
        if (diffHours <= 3) {
            return { message: 'Cannot modify reservation less than 3 hours before reservation time' };
        }

        if (body.reservation_time) {
            (reservation as any).reservation_time = new Date(body.reservation_time);
        }
        if (body.reservation_date) {
            (reservation as any).reservation_date = new Date(body.reservation_date);
        }
        if (body.guest_number !== undefined) {
            reservation.guest_number = body.guest_number;
        }
        if (body.status) {
    ...
    async delete(
        ...
        if (diffHours <= 3) {
            return { message: 'Cannot cancel reservation less than 3 hours before reservation time' };
        }
