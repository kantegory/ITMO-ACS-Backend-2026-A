...
import { createReservationViaQueue } from '../messaging/publisher';
...
async function createForRestaurant(
  ...
  if (RABBITMQ_ENABLED) {
    try {
      const reservation = await createReservationViaQueue(
        userId,
        restaurantId,
        guests_count,
      ...
      if (message.includes('not found')) return res.status(404).json({ error: 'restaurant not found' });
      if (message.includes('timeout')) return res.status(504).json({ error: message });
