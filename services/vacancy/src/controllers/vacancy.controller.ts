...
import { toVacancy, toCompany, toCompanyInternal } from "../utils/mappers";
...
export class InternalController {
  static async getCompany(req: Request, res: Response, next: NextFunction) {
    try {
      const company = await VacancyService.getCompanyById(routeParam(req.params.id));
      res.json(toCompanyInternal(company));
    } catch (e) {
      next(e);
    }
  }
