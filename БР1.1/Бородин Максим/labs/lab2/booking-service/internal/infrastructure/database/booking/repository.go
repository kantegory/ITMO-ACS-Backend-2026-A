...
// GetBookedTableIDs returns a set of table IDs (from the given list) that are
// already booked for the requested time slot.
func (r *Repository) GetBookedTableIDs(ctx context.Context, tableIDs []string, date, timeFrom, timeTo string) (map[string]bool, error) {
	if len(tableIDs) == 0 {
	...
	rows, err := r.db.QueryContext(ctx, query, pq.Array(tableIDs), date, timeFrom, timeTo)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
