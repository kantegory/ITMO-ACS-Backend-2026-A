...
func (r *Repository) GetBookedTableIDs(ctx context.Context, restaurantID, date, timeFrom, timeTo string) (map[string]bool, error) {
	query := `
	...
	rows, err := r.db.QueryContext(ctx, query, restaurantID, date, timeFrom, timeTo)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
