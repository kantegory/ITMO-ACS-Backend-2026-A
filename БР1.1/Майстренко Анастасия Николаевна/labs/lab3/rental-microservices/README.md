...
### Вариант 3. Полностью в контейнерах (ЛР3)

Каждый сервис имеет свой `Dockerfile`, общий `docker-compose.yml` собирает и
запускает все сервисы вместе с PostgreSQL и RabbitMQ:

```bash
docker compose up -d --build      # db + rabbitmq + 4 сервиса + gateway
```

Сервисы общаются друг с другом по именам в общей bridge-сети `rental-net`
(`http://catalog:8002`, `amqp://rabbitmq:5672`, `PG_HOST=db`). Наружу открыт
только gateway (`localhost:8000`). Образы: `services/<svc>/Dockerfile`,
`gateway/Dockerfile`.
