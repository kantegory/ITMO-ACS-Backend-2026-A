...
import com.petproject.propertyservice.feign.AuthServiceClient;
import com.petproject.propertyservice.repositories.PropertyImageRepository;
import com.petproject.propertyservice.repositories.PropertyRepository;
...
public class PropertyService {
    ...
    @Transactional
    public PropertyResponse createProperty(CreatePropertyRequest request, JwtPrincipal user) {
    ...
    @Transactional
    public PropertyResponse updateProperty(Long id, UpdatePropertyRequest request, JwtPrincipal user) {
        PropertyEntity property = propertyRepository.findById(id).orElseThrow(
                () -> new EntityNotFoundException("Property with id " + id + " not found")
        );

        if (!property.getUserId().equals(user.getUserId())) {
    ...
    private PropertyResponse mapToResponse(PropertyEntity request) {
        ...
        UserShortResponse user = authServiceClient.getUserById(request.getUserId());
    ...
    @Transactional
    public void deletePropertyById(Long id, JwtPrincipal user) {
        PropertyEntity property = propertyRepository.findById(id).orElseThrow(
                () -> new EntityNotFoundException("Property with id " + id + " not found")
        );
    ...
    @Transactional
    public List<ImageResponse> updateImagesOfProperty(Long propertyId, List<String> imageUrls, JwtPrincipal user) {
    ...
    @Transactional
    public void deleteImageFromProperty(Long propertyId, Long imageId, JwtPrincipal user) {
        PropertyEntity property = propertyRepository.findById(propertyId)
                                                    .orElseThrow(() -> new EntityNotFoundException("Property with id " + propertyId + " not found"));
    ...
    public Page<PropertyResponse> getAllUserProperties(JwtPrincipal user, int page, int size) {
