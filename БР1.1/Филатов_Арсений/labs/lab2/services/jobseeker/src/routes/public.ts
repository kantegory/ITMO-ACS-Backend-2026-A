...
import { catalogGetSkillsByIds, catalogValidateSkills } from "../lib/catalogClient";
...
async function requireUserId(headers: Record<string, string | undefined>, set: { status?: number }) {
...
export const jobSeekerRoutes = new Elysia({ name: "job-seeker" })
  .get("/job-seeker/profile", async ({ headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
  ...
  .get("/resumes", async ({ headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
    const items = await db.resume.findMany({ where: { userId }, orderBy: { id: "desc" } });
  ...
  .post("/resumes", async ({ body, headers, set }) => {
    ...
    const created = await db.resume.create({
      data: { userId, title: body.title, summary: body.summary, isPublic: body.isPublic ?? false },
    });
    set.status = 201;
  ...
  .get("/resumes/:resumeId/education", async ({ params, headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
    const resume = await db.resume.findUnique({ where: { id: params.resumeId } });
    if (!resume || resume.userId !== userId) {
  ...
  .post("/resumes/:resumeId/education", async ({ params, body, headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
    const resume = await db.resume.findUnique({ where: { id: params.resumeId } });
    if (!resume || resume.userId !== userId) {
    ...
    const created = await db.resumeEducation.create({
      data: {
  ...
  .patch("/resumes/:resumeId/education/:educationId", async ({ params, body, headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
    const resume = await db.resume.findUnique({ where: { id: params.resumeId } });
    if (!resume || resume.userId !== userId) {
  ...
  .delete("/resumes/:resumeId/education/:educationId", async ({ params, headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
    const resume = await db.resume.findUnique({ where: { id: params.resumeId } });
    if (!resume || resume.userId !== userId) {
  ...
  .get("/resumes/:resumeId/work-experience", async ({ params, headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
    const resume = await db.resume.findUnique({ where: { id: params.resumeId } });
    if (!resume || resume.userId !== userId) {
    ...
    return {
      items: (await db.resumeWorkExperience.findMany({ where: { resumeId: params.resumeId } })).map(mapWorkExperience),
    };
  }, {
  ...
  .post("/resumes/:resumeId/work-experience", async ({ params, body, headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
    const resume = await db.resume.findUnique({ where: { id: params.resumeId } });
    if (!resume || resume.userId !== userId) {
    ...
    const created = await db.resumeWorkExperience.create({
      data: {
        resumeId: params.resumeId,
  ...
  .patch("/resumes/:resumeId/work-experience/:workExperienceId", async ({ params, body, headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
    const resume = await db.resume.findUnique({ where: { id: params.resumeId } });
    if (!resume || resume.userId !== userId) {
  ...
  .delete("/resumes/:resumeId/work-experience/:workExperienceId", async ({ params, headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
    const resume = await db.resume.findUnique({ where: { id: params.resumeId } });
    if (!resume || resume.userId !== userId) {
  ...
  .put("/resumes/:resumeId/skills", async ({ params, body, headers, set }) => {
    const userId = await requireUserId(headers as Record<string, string | undefined>, set as { status?: number });
    if (userId == null) return apiError("UNAUTHORIZED", "Пользователь не авторизован");
    const resume = await db.resume.findUnique({ where: { id: params.resumeId } });
    if (!resume || resume.userId !== userId) {
    ...
    const validation = await catalogValidateSkills(body.skillIds);
    if (!validation.valid) {
      set.status = 400;
      return apiError("VALIDATION_ERROR", "Неизвестные skillId", { missing: validation.missing });
    }
