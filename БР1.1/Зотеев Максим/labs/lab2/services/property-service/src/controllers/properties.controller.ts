...
export const searchProperties = async (req: Request, res: Response) => {
  ...
  if (owner_id) qb.andWhere("p.ownerId = :oid", { oid: owner_id });
...
export const updateProperty = async (req: Request, res: Response) => {
  ...
  if (!p) throw notFound("Объект не найден");
  if (String(p.ownerId) !== req.user!.sub) throw forbidden("Нет прав на редактирование этого объекта");
...
export const addPhoto = async (req: Request, res: Response) => {
  ...
  if (!url) throw badRequest("url обязателен");
