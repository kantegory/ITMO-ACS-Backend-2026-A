...
// @Produce      json
// @Security     BearerAuth
// @Param        body body model.CreateChatRequest true "Данные чата"
...
func (h *ChatHandler) CreateChat(c *gin.Context) {
	...
	if req.ParticipantID == userID {
		c.JSON(http.StatusBadRequest, model.ErrorResponse{Code: 400, Message: "cannot create chat with yourself"})
		return
	}

	chat, err := h.chatRepo.CreateChat(userID, req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Code: 500, Message: "failed to create chat"})
		return
	}
