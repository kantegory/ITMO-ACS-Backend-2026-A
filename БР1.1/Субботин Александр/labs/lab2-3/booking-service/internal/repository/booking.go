...
func (r *BookingRepository) ListByTenant(tenantID string, filter model.BookingFilter) ([]model.Booking, int, error) {
	return r.listWithWhere(fmt.Sprintf("tenant_id = '%s'", tenantID), filter)
}

func (r *BookingRepository) ListByPropertyIDs(propertyIDs []string, filter model.BookingFilter) ([]model.Booking, int, error) {
	if len(propertyIDs) == 0 {
		return []model.Booking{}, 0, nil
	}
	quoted := make([]string, len(propertyIDs))
	for i, id := range propertyIDs {
		quoted[i] = fmt.Sprintf("'%s'", id)
	}
	where := fmt.Sprintf("property_id IN (%s)", strings.Join(quoted, ","))
