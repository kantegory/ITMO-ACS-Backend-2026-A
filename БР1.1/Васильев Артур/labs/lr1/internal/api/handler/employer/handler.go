...
import (
	...
	employeruc "jobsearch/internal/usecase/employer"
	"jobsearch/pkg/apperror"
	"jobsearch/pkg/httputil"
	"jobsearch/pkg/slogutil"
)

const handlerComponent = "employer"
...
func (h *Handler) GetProfile(w http.ResponseWriter, r *http.Request) {
	e, err := h.uc.GetProfile(r.Context(), middleware.UserID(r.Context()))
	if err != nil {
...
func (h *Handler) UpdateProfile(w http.ResponseWriter, r *http.Request) {
	...
	e, err := h.uc.UpdateProfile(r.Context(), middleware.UserID(r.Context()),
...
func (h *Handler) CreateVacancy(w http.ResponseWriter, r *http.Request) {
	slogutil.LogInfo(r.Context(), slogutil.LayerHandler, handlerComponent, "create vacancy started")

	v, err := decodeVacancyInput(r)
	if err != nil {
		slogutil.LogError(r.Context(), slogutil.LayerHandler, handlerComponent, "decode vacancy failed", err)
		httputil.WriteError(w, err)
		return
	}
	detail, err := h.uc.CreateVacancy(r.Context(), middleware.UserID(r.Context()), v)
	if err != nil {
		slogutil.LogError(r.Context(), slogutil.LayerHandler, handlerComponent, "create vacancy failed", err)
		httputil.WriteError(w, err)
		return
	}
	slogutil.LogInfo(r.Context(), slogutil.LayerHandler, handlerComponent, "create vacancy success",
		slog.String("vacancy_id", detail.ID.String()),
	)
...
func toVacancyDetail(v *domain.VacancyDetail) map[string]any {
	...
	m["employer"] = toEmployerDTO(&v.Employer)
