...
class AuthController extends BaseController {
    ...
    async getInternalUser(@Param('id') id: number) {
    ...
    async register(
        @Body({ type: RegisterCheck }) data: RegisterCheck,
    ): Promise<AuthResponseCheck> {
        const userFromDb = await this.repository.findOneBy({
            email: data.email,
        });
    ...
    async login(
        @Body({ type: LoginCheck }) data: LoginCheck,
    ): Promise<AuthResponseCheck> {
