...
function enrichSpecWithSecurityAndErrors(
    spec: any,
    ...
    for (const action of storage.actions) {
        const controllerRoute =
            controllerRouteByTarget.get(action.target) || '';
        ...
        const uses = storage.filterUsesWithTargetAndMethod(
            action.target,
            action.method,
        );
...
function filterSpecByMountedControllers(
    spec: any,
    options: RoutingControllersOptions,
) {
    const mountedControllers = new Set(
        (options.controllers || []).filter(
            (controller): controller is Function =>
                typeof controller === 'function',
        ),
    );

    if (mountedControllers.size === 0) {
        return spec;
    }
    ...
    for (const action of storage.actions) {
        if (!mountedControllers.has(action.target as Function)) {
            continue;
        }
        ...
        const method = String(action.type).toLowerCase();
        allowedOperations.add(`${method} ${fullPath}`);
    }

    for (const [path, pathItem] of Object.entries(spec.paths || {})) {
        for (const method of Object.keys(pathItem as Record<string, unknown>)) {
            if (!allowedOperations.has(`${method} ${path}`)) {
                delete (pathItem as Record<string, unknown>)[method];
            }
        }

        if (Object.keys(pathItem as Record<string, unknown>).length === 0) {
...
export function useSwagger(
    ...
    try {
        ...
        filterSpecByMountedControllers(spec, options);
